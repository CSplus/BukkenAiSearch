...
| 「中区で建物面積が大きい物件を探して」 | `location contains 中区`、面積条件または並び替え条件 |
| 「抵当権がない収益物件を出して」 | `income_rank = 収益物件`、`has_mortgage = no` |
...
### 6.5 気になる物件・所有者を深掘りする

所有者名による検索は重要機能だが、MVPの主導線となる代表的な初回検索ではなく、検索結果や物件詳細を確認する過程で派生する深掘り検索・関連物件探索として位置づける。

例:

### AIチャット営業支援
| 深掘りのきっかけ | 自然文 | Claudeが解釈する検索条件 |
|---|---|---|
| 検索結果に特定所有者が出てきた | 「この所有者は他にも物件を持っている？」 | `owner_name = 対象所有者` |
| 物件詳細で所有者名を確認した | 「〇〇さんの土地と建物を探して」 | `owner_name contains 〇〇`、必要に応じて `registry_type in 土地, 建物` |
| 候補物件の関連性を確認したい | 「同じ所有者の物件を一覧にして」 | 現在選択中物件の `owner_name` を条件に関連物件検索 |

顧客ヒアリング内容を自然文で入力し、AIが営業支援回答を返す。
この深掘り検索も `ai_search` の同一または派生 `Conversation` で扱い、検索条件・Message・候補保存元を追跡できるようにする。

### 6.6 候補物件として保存する

自然言語検索、追加絞り込み、気になる物件・所有者の深掘りを経て、有望な物件を `CandidateProperty` として保存する。
